...
What is different than a regular YOLOv5 detector which typically uses the detect.py code developed by Ultralytics, this code leveraged the OpenCV DNN backend as the DL backbone, therefore an onnx-format weight file was converted from the pytorch's dot-pt-format which was taken by the transfer learning proccess. So, generally it is an indepentent app for detecting pill bottles.  
